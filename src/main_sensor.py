...
for omega in range(Omega):
    for c in C:
        ...
        #             var_x[j, i] = model.addVar(lb=0, ub=0, vtype=GRB.BINARY, name='x.' + str(j) + '.' + str(i))
        # model.update()
        # model.setObjective(quicksum(model.getVarByName('x.' + str(0) + '.' + str(i)) for i in range(N)) * c[1]
        #                    + quicksum(model.getVarByName('x.' + str(1) + '.' + str(i)) for i in range(N)) * c[1])
        # '''========================'''
        # for r in range(R):
        #     for s in range(R):
        ...
        #                 >= 1, 'c1.' + str(r) + '.' + str(s))
        # for r in range(R):
        #     model.addConstr(quicksum(W[i][r] * (model.getVarByName('x.0.' + str(i))) for i in range(N)) >= 1, 'c2-1.' + str(r))
        #
        ...
        '''solve model'''
        model.setParam(GRB.Param.LogFile, f'RD_appro/{node}.{N}.{R}/'+net+f'_logging.{omega}.{c}.{node}.{N}.{R}.log')
        model.setParam('TimeLimit', timeLimit)
        ...
        with open(f'RD_appro/{node}.{N}.{R}/'+net+f'_results.{omega}.{c}.{node}.{N}.{R}.txt','w') as file:
