...
def collect(ins_dir,q,sol_dir,log_dir,bg_dir,settings):
    ...
    while True:
        ...
        # skip if no solutions / empty objs
        objs = sol_data.get('objs') if isinstance(sol_data, dict) else None
        if objs is None or (hasattr(objs, 'size') and objs.size == 0) or (hasattr(objs, '__len__') and len(objs) == 0):
            print(f"Skipping {filename}: no solutions found")
            continue
