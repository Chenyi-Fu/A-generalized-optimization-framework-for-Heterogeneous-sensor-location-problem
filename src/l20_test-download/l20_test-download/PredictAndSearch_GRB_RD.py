...
for ins_num in range(TestNum):
    ...
    for i in range(len(scores)):
        if count0 < k_0:
            ...
            fixer += 1
    # for i in range(len(scores)):
    #     if scores[i][2] < alpha_threshold[0]:
    #         scores[i][3] = 0
    #         fixer += 1
    #     elif scores[i][2] > alpha_threshold[1]:
    #         scores[i][3] = 1
    #         fixer += 1
    #     else:
    #         continue
    ...
    with open(f'{log_folder}/{test_ins_name}.log', 'w'):
        pass

    #read instance
    gurobipy.setParam('LogToConsole', 1)  # hideout
    if ins_name_to_read.endswith('.gz'):
        tmp_path = gz2mps(ins_name_to_read)
        m = gurobipy.read(tmp_path)
        os.remove(tmp_path)
    else:
    ...
    if m.status == GRB.INFEASIBLE:
        print(f'instance: {test_ins_name} is infeasible after fixing {fixer} variables.')
        continue
    if m.status == GRB.TIME_LIMIT and m.solCount == 0:
        print(f'instance: {test_ins_name} reach time limit without feasible solution.')
        continue
    ### save the solution
    mvars = m.getVars()
    oriVarNames = [var.varName for var in mvars]
    sol = np.array([var.X for var in mvars], dtype=np.float32)
    ...
    pickle.dump(sol_data, open(f"{log_folder}/{test_ins_name}_result.p", "wb"))
    end_time = time.time()
    with open(f'{log_folder}/{test_ins_name}.log', 'a') as f:
