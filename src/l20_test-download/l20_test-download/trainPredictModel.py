...
NB_EPOCHS = 2000
BATCH_SIZE = 2
NUM_WORKERS = 8 if torch.cuda.is_available() and os.name == 'posix' else 0
WEIGHT_NORM=100
...
# preload the dataset
# 第一步：预加载所有图到内存中（只做一次）
train_graphs = [GraphDataset(train_files).get(i) for i in range(len(train_files))]
valid_graphs = [GraphDataset(valid_files).get(i) for i in range(len(valid_files))]

# 第二步：用轻量的内存 Dataset 封装
class InMemoryListDataset(torch.utils.data.Dataset):
...
train_data = InMemoryListDataset(train_graphs)
valid_data = InMemoryListDataset(valid_graphs)
...
def EnergyWeightNorm(task):
    if task=="IP" or task=="RD":
        return 1
    elif task=="WA":
        return 100
    elif task == "IS" or task[:3] == "MIS":
        return -1
    elif task == "CA":
        return -1000
    elif task == "MK" or task[:2] == 'MK':
        return 10
    else:
        return 1000
...
def train(predict, data_loader, optimizer=None,weight_norm=1):
    ...
    with torch.set_grad_enabled(optimizer is not None):
        roc_test = []
        #data_wait_time = time.time()
        for step, batch in enumerate(data_loader):
            #print(f"[Step {step}] Data loading wait: {time.time() - data_wait_time:.2f}s")
            ...
            for ind,(sols,vals) in enumerate(zip(target_sols,target_vals)):
                ...
                n_vals = vals
                # for test
                #weight_norm = torch.std(n_vals) + 1e-6
                #weight_norm = 1
                n_vals = n_vals - n_vals.min()
...
if __name__ == "__main__":
    ...
    for epoch in range(NB_EPOCHS):
        ...
        #print(f"Train time: {time.time() - begin:.2f}s")
