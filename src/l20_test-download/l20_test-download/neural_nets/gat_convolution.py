...
class GATConvolution(torch_geometric.nn.MessagePassing):
    ...
    def __init__(self,
        ...
        self.lin_proj = torch.nn.Linear(emb_size * 2 + 1, self.heads * self.out_channels * 3, bias=False) # added
    ...
    def message(self, x_j, x_i,
                index,
                edge_features):
        x = torch.cat([x_i, x_j, edge_features], dim=-1) # 64 * 2 + 1
        x = torch.nn.functional.leaky_relu(x, self.negative_slope)
        x = self.lin_proj(x) # 64 * 8
