...
def gz2mps(ins_name):
    ...
    with gzip.open(ins_name, 'rb') as gz_file:
        with tempfile.NamedTemporaryFile(suffix='.mps', delete=False) as tmp_file:
...
def get_a_new2(ins_name):
    ...
    m = scp.Model()
    m.hideOutput(True)
    if ins_name.endswith('.gz'):
        tmp_file_path = gz2mps(ins_name)
        m.readProblem(tmp_file_path)
