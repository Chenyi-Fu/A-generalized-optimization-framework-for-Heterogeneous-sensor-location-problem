...
from neural_nets.gat_convolution import GATConvolution
...
class GNNPolicy(torch.nn.Module):
    def __init__(self):
        ...
        # self.conv_v_to_c2 = GATConvolution()
        # self.conv_c_to_v2 = GATConvolution()
    ...
    def forward(
        ...
        variable_features0 = variable_features
        constraint_features0 = constraint_features
        ...
        # resNet structure
        #variable_features += variable_features0
...
class GraphDataset(torch_geometric.data.Dataset):
    ...
    def get(self, index):
        ...
        constraint_features[torch.isnan(constraint_features).bool()] = 1
        ...
        #     torch.tensor(variable_features, dtype=torch.float32, device=DEVICE)
        # )
        graph = BipartiteNodeData(
            constraint_features.clone().detach(),
            edge_indices.clone().detach(),
            edge_features.clone().detach(),
            variable_features.clone().detach(),
        )
        ...
        graph.solutions = torch.tensor(sols, dtype=torch.float32, device=DEVICE).reshape(-1)

        #graph.objVals = torch.FloatTensor(objs)
        graph.objVals = torch.tensor(objs, dtype=torch.float32, device=DEVICE)
